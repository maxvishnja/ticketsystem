...
class Agent extends User
{
    ...
    {
        return $query->where('ticketit_agent', '1')->lists('last_name', 'id')->toArray();
    }
    ...
    public static function isAgent($id = null)
    {
        ...
        if (Sentry::check()) {
            if (Sentry::getUser()->ticketit_agent) {
                return true;
            }
        }
    }
    ...
    {
        if (Sentry::check()) {
            if (Sentry::getUser()->ticketit_admin) {
                return true;
            } elseif (!is_null(Setting::where('slug', 'admin_ids')->first()) && in_array(Sentry::getUser()->id, Setting::grab('admin_ids'))) {
                return true;
            }
        }
    }
    ...
    {
        if (Sentry::check() && Sentry::getUser()->ticketit_agent) {
            if (Sentry::getUser()->id == Ticket::find($id)->agent->id) {
                return true;
            }
        }
    }
    ...
    {
        if (Sentry::check()) {
            if (Sentry::getUser()->id == Ticket::find($id)->user->id) {
                return true;
            }
        }
    }
    ...
    {
        $user = self::find(Sentry::getUser()->id);
